...
def searchMahasiswa():
    ...
    if len(mahasiswa) > 0:
        ...
        if not nim.isdigit(): # jika inputan bukan digit, maka kasih pesan.
            clearScreen() # membersihkan layar terminal
...
def createMahasiswa():
    ...
    with open(fileData, mode='a', newline='') as file:
        ...
        # Kondisi, jika salah satu inputan ada yang kosong
        if len(nama) == 0 and len(jurusan) == 0 and len(prodi) == 0 and len(kelas) == 0:
            ...
            createMahasiswa() # ulang lagi dari awal
        else: # jika inputan diisi semua
            if mahasiswa == []: # Jika data mahasiswa kosong maka buat header table
                writer.writeheader() # buat header table
